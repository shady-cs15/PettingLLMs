...
The Agent Execution Engine is the central orchestrator in pettingllms that manages interactions between agents and environments. It handles batched execution, trajectory collection, and provides both synchronous and asynchronous execution modes for scalable RL training.
...
2. **Parallel Execution**: Each agent-environment pair performs trajectory generation asynchronously. pettingllms supports the OpenAI Completions interface for LLM inference and structures requests sent to the inference engine endpoint.
