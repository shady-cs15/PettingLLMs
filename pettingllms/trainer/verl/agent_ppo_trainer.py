...
from pettingllms.engine.agent_execution_engine import AsyncAgentExecutionEngine
...
class AgentPPOTrainer:
    def __init__(
        ...
        # Initialize the RayPPOTrainer group
        self.num_ppo_groups = config.actor_rollout_ref.agent_num
        self.ppo_trainer_group = []
        
        for i in range(self.num_ppo_groups):
            ...
            self.ppo_trainer_group.append(ppo_trainer)
        
        # Use the first trainer as the primary one for backward compatibility
        self.primary_trainer = self.ppo_trainer_group[0]
        ...
        # Delegate common attributes to primary trainer
        self.config = self.primary_trainer.config
        self.tokenizer = self.primary_trainer.tokenizer
        self.hybrid_engine = self.primary_trainer.hybrid_engine
        self.use_critic = self.primary_trainer.use_critic
        self.use_reference_policy = self.primary_trainer.use_reference_policy
        self.use_rm = self.primary_trainer.use_rm
        self.total_training_steps = self.primary_trainer.total_training_steps
        self.train_dataloader = self.primary_trainer.train_dataloader
        self.val_dataloader = self.primary_trainer.val_dataloader
        
        # Worker groups
        self.actor_rollout_wg = self.primary_trainer.actor_rollout_wg
        self.rollout_wg = self.primary_trainer.rollout_wg
        self.critic_wg = self.primary_trainer.critic_wg
        self.ref_policy_wg = self.primary_trainer.ref_policy_wg
        self.rm_wg = self.primary_trainer.rm_wg
        self.actor_wg = self.primary_trainer.actor_wg
        
        # Other important attributes
        if hasattr(self.primary_trainer, 'async_rollout_manager'):
            self.async_rollout_manager = self.primary_trainer.async_rollout_manager
        
        # Reward functions
        self.reward_fn = self.primary_trainer.reward_fn
        self.val_reward_fn = self.primary_trainer.val_reward_fn
    ...
    # Property delegation for mutable attributes
    @property
    def global_steps(self):
        return self.primary_trainer.global_steps
    
    @global_steps.setter
    def global_steps(self, value):
        self.primary_trainer.global_steps = value
        # Sync to all trainers in the group
        for trainer in self.ppo_trainer_group:
            trainer.global_steps = value

    # Delegate methods to primary trainer
    def _load_checkpoint(self):
        """Delegate checkpoint loading to primary trainer."""
        return self.primary_trainer._load_checkpoint()
    
    def _save_checkpoint(self):
        """Delegate checkpoint saving to primary trainer."""
        return self.primary_trainer._save_checkpoint()
    
    def _balance_batch(self, batch, metrics=None):
        """Delegate batch balancing to primary trainer."""
        return self.primary_trainer._balance_batch(batch, metrics)

    def init_workers(self):
        # Initialize workers for all trainers in the group
        for trainer in self.ppo_trainer_group:
