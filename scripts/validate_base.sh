...
#)
models=(
  "/home/lah003/workspace/verl_efficient/checkpoints/verl_examples/gsm8k/code_1.7B_two_policies_livecodebench/global_step_151/actor/checkpoint "
)
...
for model in "${models[@]}"; do
  ...
  python3 -m pettingllms.scripts.async_vllm_code_eval \
    --config-path "$config_path_code" --config-name "$config_name_code" \
    +parallel=true \
